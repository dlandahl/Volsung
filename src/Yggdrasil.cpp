...
//It's a template function, and the subclass of AudioObject you want to create is passed as a template argument.
...
{
	//This static_assert checks at compile time that the type that has been passed as the template argument
	//	is derived from the abstract AudioObject class.
	static_assert(std::is_base_of<AudioObject, obj>::value, "no");
	
	//This creates an anonymous instance of the type that has been passed into the template, and maps the name
	//	which has been passed to a pointer to that instance.
	symbols[name] = new obj(args);
	
	//This way, individual oscillators or effects that are created don't have a variable or symbol in the code,
	//	but a string name by which they can be accessed from the symbol table.
}

//This function takes in two pointers to AudioObjects and two integers which are the indeces of the input and the output
//	of each of them you want to connect.
...
{
	//The connections that are connected to an output or input are std::vectors, so that any number of connections can be
	//	linked to one input or output.
	
	//This code adds a value to the vector of connections on the right input and output on the two *AudioObjects.
	a->outputs[out].connections.push_back({});
	b->inputs [in] .connections.push_back({});
	
	//This links both the new positions in the vector to a new anonymous pointer to an AudioConnector, which allows them to
	//	communicate.
...
int main()
{
	...
	//Create an oscillator called "mod" and pass "1" as an argument. This argument is interpreted as the frequency
	//	by the constructor of the OscillatorObject. OscillatorObject is derived from AudioObject.
	//Thereby an OscillatorObject IS an AudioObject.
	create_object<OscillatorObject>("mod", st, "1");
	//Create another oscillator with a higher frequency and place it in the symbol table so it can be referred to later.
	...
	//Execute the run function on all objects in the symbol table st ten-thousand times. These run functions should
	//	contain the code that will make the object perform its function.
